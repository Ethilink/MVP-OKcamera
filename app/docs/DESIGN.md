...
| D8 | Identity = the **session id** the tracker emits, verbatim. Labels are `"Instrument {id}"` | single class; re-id is model-side. **Resolved 2026-07-08:** ids survive absence via **track linking** behind the seam (tracker-interface § "Identity semantics") — linked tracks re-emit the original id within ≤ 1 s of return; `Session` adds a 1 s **entry debounce** so provisional pre-link ids leave no trace. **Updated 2026-07-15:** the linker is **built and owned in `model/`** (`SessionLinker`, composed by `load_tracker()`) — not Constantijn's. Two deltas the app must respect: (a) the roster is **frozen at Start** — a track that matches no enrolled identity is **Unknown**, is never labelled `"Instrument N"`, and stays out of Usage and Completeness (see D8a); (b) the session id is currently **Deep OC-SORT's raw counter**, so N is stable *within* a recording but arbitrary *across* one — a tray of 8 can show "Instrument 10". Making N mean the specimen is [wayfinder T08](../../docs/wayfinder/session-linker/tickets/T08-gallery-binding.md), not an app change. |
| D8a | **Unknown objects** render gray, outside Usage and Completeness; a link in flight shows a **Pending** spinner | 2026-07-15, closing wayfinder T06. Freezing the roster at Start is what keeps an audience member's phone from becoming "Instrument 7" and counting as present. **Grilled 2026-07-15 (Bram), build in flight (wayfinder T10):** the app learns the roster from the seam — `InstrumentTracker` gains a read-only `roster` property (route (b); route (a)'s app-derived Start snapshot rejected for its ~0.7 s coordination gap with the linker's enrolment freeze). Unknowns are **video-only**: gray mask + ~1 s resolving spinner on the feed, no panel row, never in `/status.recording.instruments`, Usage, or Completeness. Each roster instrument's `/status` entry carries its fixed mask **colour** so the panel swatch and the overlay can never drift; a returned instrument regains its colour because the linker re-emits its original session id. See `model/docs/linker-design.md` §7 |
