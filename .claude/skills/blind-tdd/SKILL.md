...
Each sub-agent invokes its own skills as its first action. Put the instruction in the spawn prompt: "First invoke the `<skill>` skill(s), then…". Codex is prompt-composed by the orchestrator (it has no Claude Skill tool).
